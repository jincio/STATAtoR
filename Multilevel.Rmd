...
Multilevel model takes into advantage the nested data's structure. Where there are data at differente levels of agregation. 
...
Where the intercept does not have any independient variable, the "mixed" form can be represented as:

$$
Y_{it}=\beta_{00}+\beta_{1}X_{1it}..+\zeta_{0i}+ e_{it}
$$

Which is the same as the Random Effect Model we explained [here](#RE)
...
- For mixed model we will use the function ```lmer``` from the package **lm4** 

- This case the model seems similar to what we have done before. The "fixed effect part" comes first ```dg01i~l203+l212``` and we add the randon effect ```(1|cc_2)``` for the intercept.
...
We get the same output than STATA.
...
- First we need to standarized the variable TIME 
...
- First we create the variable ```yearnum2`` that recodes the yearnum so there is a zero point at the begining of the series. 

```{r}
demo$yearnum2<-demo$yearnum-1
```

- We don't need to create the interactions variables. We can add the interaction straight to the model ```l203*yearnum2+ l225*yearnum2```. 
...
- The model has three random effects and eigth fixed effects. 


# SEM Multilevel Models <a name="SEM_Multilevel"></a>

Here we are goint to estimate a Latent Growth Model using the SEM framework. 
