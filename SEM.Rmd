...
Below is how an autoregressive model would look with three waves (using the SEM builder). To estimate the model, we need to clik "Estimate" (right on the horizontal menu)
...
- We will explore these arguments progresively. Lets start changing the style to "lisrel", this option changes the plot and shows a single headed edges with no node as origin."Estimates" add the estimates to the arrows.
...
- Similar to the autorregresive model (above), the cross-lagged model has structural effects. For instance ```pid2002 ~ pid2000 + app2000```
...
- Now, we ask for the results. Adding the option ```standardized=TRUE``` add the standarized coefficients (last column)
...
# Meaurement error model <a name="ME"></a>

Measurement erros is especial problematic for panel analysis. In SEM framework we can deal with this in a simple way. For instance, lets take our **Autoregresive model** and lets model its measurement error. We will model the Wiley-Wiley solution ( asuming equal measurement error)  in order to have an identified model.
...
- We can obtain our variance-covariance matriz in R. 
...
- Usin the SEM builder we create the latent variables for each variable and constraint its error variance to 0.48. 

- The red box shows where you can impose a costraint to the variance error variance.
...
- To fix the error variance we multiply it with 0.48 
...
For this section we will use the XXXX dataset, it is in a wide format. You can review about data's formats (wide and long) here [Data organization](#cleaning)
...
In this model the *latent* U correlates with the indepent variables and havea constant effect on the dependent variable. 
...
Summarizing: 
- This model has a **latent** U term that represent the unobserved heterogeneity that do not change with time. 
...
- We also set the variance of the dependent variable constant in each time. [ask why]
...
- Then we set the models for instance```dg02i1~c*aid1001```. Here **dg02i1** is the dependent variable and **aid1001** es the dependent variable. We are multiplying every regression with *c*, then the effects are constrained. 

- We would like to constraint the intercepts to. We do it multipling the 1 with a constant. ``` dg02i1~x*1``` 

- Finally, we also constrained the variance with a constant ```dg02i1~~x*dg02i1```
